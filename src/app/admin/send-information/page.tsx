...
export default function SendInformationPage() {
  ...
  const load = async () => {
    try {
      ...
      setItems([]);
      setError(
        err instanceof Error ? err.message : "Failed to load send informations",
      );
    } finally {
      setIsLoading(false);
    }
              ...
              <div className="column-visibility-wrap">
                ...
                {showColumnMenu ? (
                  <div className="dropdown-menu show column-visibility-menu">
                    ...
                    {renderColumnToggleItem("Title", "title")}
                    {renderColumnToggleItem("Message", "message")}
                    {renderColumnToggleItem("Image", "image")}
                    {renderColumnToggleItem("Web Link", "webLink")}
                    {renderColumnToggleItem("Status", "status")}
                ...
                <thead>
                  <tr>
                    ...
                    {visibleColumns.title ? (
                      <th>{renderSortHeader("Title", "title")}</th>
                    ) : null}
                    {visibleColumns.message ? (
                      <th>{renderSortHeader("Message", "message")}</th>
                    ) : null}
                    {visibleColumns.image ? <th>Image</th> : null}
                    {visibleColumns.webLink ? (
                      <th>{renderSortHeader("Web Link", "web_link")}</th>
                    ) : null}
